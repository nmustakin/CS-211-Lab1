...
void ijk(const double *A, const double *B, double *C, const int n) 
{
  ...
  for(i=0; i<n; i++){
    for(j=0; j<n; j++){
      register double r = C[i*n+j];
      for(k=0; k<n; k++) r+= A[i*n+k]*B[k*n+j];
      C[i*n+j] = r; 
    }
  }
}
  ...
  for(i = 0; i < n; i+=b){
    for (j = 0; j < n; j+=b){
	  ...
	  for (j1 = j; j1 < j+b; j1++) {
	    ...
	    for (k1 = k; k1 < k+b; k1++) r += A[i1*n + k1]*B[k1*n + j1];
	    C[i1*n+j1]=r;
	  }
	}
      }
...
void kij(const double *A, const double *B, double *C, const int n) 
{
  ...
  for(k=0; k<n; k++){
    for(i=0; i<n; i++){
      ...
      for(j=0; j<n; j++) C[i*n+j] += r * B[k*n+j]; 
    }
  }
}
  ...
  for(k = 0; k < n; k+=b){
    for (i = 0; i < n; i+=b){
	  ...
	  for (i1 = i; i1 < i+b; i1++) {
	    ...
	    for (j1 = j; j1 < j+b; j1++) C[i1*n+j1] += r*B[k1*n + j1];
	  }
	}
      }
    }
...
void jki(const double *A, const double *B, double *C, const int n) 
{
  ...
  for(j=0; j<n; j++){
    for(k=0; k<n; k++){
      ...
      for(i=0; i<n; i++) C[i*n+j] += A[i*n+k] * r; 
    }
  }
}
  ...
  for(j = 0; j < n; j+=b){
    for (k = 0; k < n; k+=b){
	  ...
	  for (k1 = k; k1 < k+b; k1++) {
	    ...
	    for (i1 = i; i1 < i+b; i1++) C[i1*n+j1] += A[i1*n + k1] * r;
	  }
	}
      }
    }
